...
# Define deques to hold recent readings for monitoring conditions
smoker_window = deque(maxlen=5)  # 2.5 minutes window (5 readings)
foodA_window = deque(maxlen=20)  # 10 minutes window (20 readings)
foodB_window = deque(maxlen=20)  # 10 minutes window (20 readings)
...
def foodA_callback(ch, method, properties, body):
    ...
    timestamp, temperature = struct.unpack('!df', body)
    timestamp_str = datetime.fromtimestamp(timestamp).strftime("%m/%d/%y %H:%M:%S")
    print(f" [x] Received from food A queue: {timestamp_str} - Temperature: {temperature}F")

    # Add the new temperature reading to the deque
    foodA_window.append(temperature)

    # Check if the temperature change is 1F or less within the last 10 minutes
    if len(foodA_window) == foodA_window.maxlen:
        if max(foodA_window) - min(foodA_window) <= 1:
...
def foodB_callback(ch, method, properties, body):
    ...
    timestamp, temperature = struct.unpack('!df', body)
    timestamp_str = datetime.fromtimestamp(timestamp).strftime("%m/%d/%y %H:%M:%S")
    print(f" [x] Received from food B queue: {timestamp_str} - Temperature: {temperature}F")

    # Add the new temperature reading to the deque
    foodB_window.append(temperature)

    # Check if the temperature change is 1F or less within the last 10 minutes
    if len(foodB_window) == foodB_window.maxlen:
        if max(foodB_window) - min(foodB_window) <= 1:
